...
def convert_trades_to_ohlcv_unified(trade_data, symbol, timeframe='4S'):
    ...
    try:
        ...
        ohlcv = df_trades.resample(timeframe).agg({
...
def download_stock_data_rsi(symbol, start_date, end_date):
    """Legacy function - use download_stock_data(symbol, start_date, end_date, 'rsi') instead"""
    return download_stock_data(symbol, start_date, end_date, 'rsi')

def download_stock_data_vwap(symbol, start_date, end_date):
    """Legacy function - use download_stock_data(symbol, start_date, end_date, 'vwap') instead"""
    return download_stock_data(symbol, start_date, end_date, 'vwap')
...
def convert_trades_to_ohlcv_1min(trade_data, symbol):
    ...
    try:
        ...
        print(f"   📊 Converting {len(df)} trades to 1-minute OHLCV format for {symbol}")
        print(f"   📋 Trade data columns: {list(df.columns)}")
        ...
        ohlcv = df_trades.resample('1T').agg({
            'Price': ['first', 'max', 'min', 'last'],  # Open, High, Low, Close
            'Volume': 'sum'
        }).dropna()
